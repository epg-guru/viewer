...
import { sliceStringOrUndefined } from './xmltv/columnar';
...
export function buildCategoryColorMap(index: EpgIndex): CategoryColorMap {
  ...
  for (let i = 0; i < index.programmes.count; i++) {
    const category = sliceStringOrUndefined(index.programmes.category, i);
